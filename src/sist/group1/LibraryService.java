...
public class LibraryService {
	...
	public void adminMenu(Scanner sc) {
		while(true){
			...
			if(input == 0) break;
			switch(input) {
			case 1:this.bookMng(sc);break;
			case 2:/*회원 관리 메뉴 메소드 호출*/break;
			default : System.out.println("알 수 없는 입력입니다. 다시 입력해주세요.");
			}
		}
	}
	//관리자 메뉴 중 도서관리 메뉴의 하위 메뉴
	public void bookMng(Scanner sc) {
		while(true){
			System.out.println("도서관리 메뉴입니다.");
			System.out.println("1.전체도서 2.도서 검색 3.대출중 도서 4.연체중 도서  0.나가기");
			int input = sc.nextInt();
			if(input == 0) break;
			switch(input) {
			case 1:System.out.println(this.dao.viewAllBooks());break;
			case 2:/*도서검색 메소드 호출*/break;
			case 3:/*대출중 도서 메소드 호출*/break;
			case 4:System.out.println(this.dao.viewOverdueBooks());break;
	...
	//관리자 메뉴 중 회원관리 메뉴의 하위 메뉴
		public void userMng(Scanner sc) {
			while(true){
				System.out.println("회원관리 메뉴입니다.");
				System.out.println("1.전체회원 2.회원검색 0.취소");
				int input = sc.nextInt();
				if(input == 0) break;
				switch(input) {
				case 1:System.out.println(this.dao.viewAllUsers());;break;
				case 2:/*회원검색 메소드 호출*/break;
				case 0:System.out.println("취소합니다.");break;
				default : System.out.println("알 수 없는 입력입니다. 다시 입력해주세요.");
				}
			}
